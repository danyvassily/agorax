-- Preserve all histories. Reject incomplete room identities; serialize overlapping groups.
...
begin
  ...
  from chosen
  where (select count(*) from reserved where reserved.family_id = chosen.family_id) = cardinality(v_profiles)
