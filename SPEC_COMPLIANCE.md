# Agorax / AGORAX — Specification Compliance
...
## Piliers Produit & Game Design AGORAX

| REQUIREMENT | STATUS | IMPLEMENTATION | TEST | EVIDENCE |
|---|---|---|---|---|
| Nom centralisé (spec §1) | VERIFIED | `src/lib/brand.ts` — AGORAX / Agorax | grep hors brand.ts | ✅ |
| Repo officiel (spec §2) | VERIFIED | `github.com/danyvassily/Agorax` cloné, remote officiel | `git remote -v` | ✅ |
| Mode Majeur AGORAX (spec §2–§7) | VERIFIED | `src/lib/game/agorax-engine.ts` + `agorax-game.tsx` | `tests/agorax-engine.test.ts` | ✅ |
| Mode A : Tour par tour & Vol (+50 pts) | VERIFIED | `agorax-game.tsx` (15s chrono, fast bonus +50, vol/steal 5s +50) | tests engine | ✅ |
...
| Élimination Hybride C : Le Cut & Sauvetage (Decision #1) | VERIFIED | `le-cut-modal.tsx` + `processLeCutLeaderboard` | `tests/agorax-engine.test.ts` | ✅ |
| Finale Signature : La Ligne 9 positions & DOUBLE (Decision #4) | VERIFIED | `la-ligne.tsx` + `processLaLigneAnswer` (9 positions, DOUBLE tous les 3 tours, 90s chrono) | `tests/agorax-engine.test.ts` | ✅ |
