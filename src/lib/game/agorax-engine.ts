/**
 * AGORAX / Agorax — Master Game Engine (spec §2–§8, §28, Decisions 1–6)
 *
...
export type AgoraxDuration = "express" | "classic"; // express = 3 manches + cut + ligne, classic = 5 manches + cut + ligne

export type AgoraxPhase =
  | "lobby"
  | "round-intro"
  | "turn-active"
  | "turn-steal"
...
export interface AgoraxGameState {
...
export function createInitialAgoraxState(options: {
  ...
  const players: AgoraxPlayer[] = options.players.map((p, i) => ({
    id: p.id,
    name: p.name,
 ...
 */
export function processLeCutLeaderboard(players: AgoraxPlayer[]): {
  sortedPlayers: AgoraxPlayer[];
  topTwo: [AgoraxPlayer, AgoraxPlayer];
  challengers: [AgoraxPlayer, AgoraxPlayer] | null;
} {
  const sorted = [...players].sort((a, b) => b.score - a.score);
  const topTwo: [AgoraxPlayer, AgoraxPlayer] = [sorted[0], sorted[1]];
  const challengers: [AgoraxPlayer, AgoraxPlayer] | null = sorted.length >= 4 ? [sorted[2], sorted[3]] : null;
