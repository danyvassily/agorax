...
export const MediaMetadataSchema = z.object({
  type: z.enum(["image", "audio", "svg", "youtube"]),
  url: z.string().url(),
...
export const QuestionSchema = z
  .object({
    ...
    /** Media attaché à la question (Image, SVG, Audio) */
  ...
  .superRefine((q, ctx) => {
    ...
    if (goodAnswer && goodAnswer.length > 2 && qLower.includes(goodAnswer)) {
      ...
      const directHomonym = new RegExp(
        `^(?:quelle est la capitale de|what is the capital of)\\s+${escaped}\\s*\\??\\s*$`,
      ).test(qLower);
      const reverseHomonym = new RegExp(
        `^${escaped}\\s+(?:est la capitale de quel pays|is the capital of which country)\\s*\\??\\s*$`,
      ).test(qLower);
