...
export function KawaiiMascot({theme,size=72,className='',alt,animation='none'}:{theme:KawaiiTheme;size?:number;className?:string;alt?:string;animation?:'float'|'wobble'|'bounce'|'dance'|'celebrate'|'shake'|'pop'|'none'}) {
  const character=CHARACTERS.find(c=>c.id===(themes[theme]??theme))??CHARACTERS[0];
  return <span className={`jx-mascot ${animation==='celebrate'?'jx-celebrate':''} ${className}`} style={{width:size,height:size}}><Image src={characterImage(character.id)} width={size} height={size} alt={alt??character.name} className="h-full w-full object-contain" /></span>;
}
