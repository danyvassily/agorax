...
/**
 * Agorax — Dilemmes (Would You Rather)
 * Les joueurs tranchent à tour de rôle, puis le groupe discute.
 */
import { useState } from "react";
import { useRouter } from "next/navigation";
import { pickWyrPair, WYR_CATEGORY_LABELS, WYR_CATEGORY_LABELS_EN, type WouldYouRatherPair } from "@/lib/game/wyr-data";
    ...
    return (
      <main key={sessionKey} className="mx-auto flex min-h-dvh max-w-xl flex-col items-center justify-center px-6 text-center animate-rise">
        ...
        <h1 className="mt-3 text-[26px] font-bold text-fp-text">
          {isEn ? "Dilemmas completed" : "Dilemmes terminés"}
        </h1>
        <p className="mt-1 text-[14px] text-fp-text-dim">
          {isEn
            ? `${totalRounds} dilemmas resolved — great discussion!`
            : `${totalRounds} dilemmes tranchés — belle discussion !`}
        </p>
        <div className="mt-8 flex w-full max-w-sm gap-3">
          <button type="button" onClick={() => router.push("/")} className="fp-btn-secondary flex-1 py-3 text-[15px]">
            {isEn ? "Home" : "Accueil"}
          </button>
          <button type="button" onClick={replay} className="fp-btn-primary flex-1 py-3 text-[15px]">
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-2xl flex-col px-4 pb-10 pt-3">
      <div className="flex items-center justify-between">
        <button type="button" onClick={() => router.push("/")} className="fp-btn-ghost inline-flex items-center gap-0.5 px-2 py-1 text-[15px]" aria-label={isEn ? "Quit" : "Quitter"}>
          <ChevronLeft className="h-5 w-5" />
          {isEn ? "Quit" : "Quitter"}
        </button>
        <PillBadge>
          {round}/{totalRounds} · {isEn ? (WYR_CATEGORY_LABELS_EN[pair.category] ?? pair.category) : (WYR_CATEGORY_LABELS[pair.category] ?? pair.category)}
        </PillBadge>
      </div>
      ...
      <div className="mt-10 text-center">
        {active && (
          <p className="mb-2 inline-flex items-center gap-2 text-[15px] font-medium text-fp-text">
            ...
            {isEn ? `Your turn, ${active.name}` : `À toi, ${active.name}`}
          </p>
        )}
        <h1 className="text-[20px] font-semibold text-fp-text">
          {isEn ? "Would you rather…" : "Tu préfères…"}
        </h1>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 relative items-stretch">
        ...
        >
          ...
          <p className="mt-3 text-[17px] sm:text-[19px] font-semibold leading-snug text-fp-text">
            {isEn ? (pair.optionAEn ?? pair.optionA) : pair.optionA}
          </p>
        </button>
        ...
        >
          ...
          <p className="mt-3 text-[17px] sm:text-[19px] font-semibold leading-snug text-fp-text">
            {isEn ? (pair.optionBEn ?? pair.optionB) : pair.optionB}
          </p>
        </button>
      </div>
      ...
      {chosen && (
        <p className="animate-pop mt-5 text-center text-[14px] text-fp-text-dim">
          {isEn
            ? `${active?.name ?? "The player"} chose Option ${chosen} — time to debate!`
