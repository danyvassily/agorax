...
import type { AgoraxPlayer } from "@/lib/game/agorax-engine";
import { sound } from "@/lib/audio/sound-engine";
import { checkTypedAnswer } from "@/lib/game/agorax-engine";
...
export function BuzzerScreen({
  ...
  useEffect(() => {
    ...
    const stop = startQuestionCountdown(8, (remaining) => {
    ...
    return () => { stop(); if (feedbackTimer.current) clearTimeout(feedbackTimer.current); };
  }, [isAnswering, lockedPlayerId, question.id]);
  ...
  function handleChoiceQCM(index: number) {
    if (!lockedPlayerId || submitted.current) return;
    submitted.current = true;
    ...
    feedbackTimer.current = setTimeout(() => {
      submitRef.current(lockedPlayerId, correct);
      setAnsweredIndex(null);
    }, 1200);
  }
  ...
  function handleTypedSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!lockedPlayerId || !typedInput.trim() || submitted.current) return;
    submitted.current = true;
