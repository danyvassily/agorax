...
/**
 * Agorax — AGORAX (mode compétitif signature)
...
import {
  ...
  type AgoraxGameState,
  type AgoraxPlayer,
} from "@/lib/game/agorax-engine";
...
export function AgoraxGame() {
  const router = useRouter();
  const config = useGameStore((s) => s.config);
  const { entries, addReport } = useHistoryStore();
  const language = useLanguageStore((s) => s.language);

  const [gameState, setGameState] = useState<AgoraxGameState | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const initial = createInitialAgoraxState({
  ...
  const advanceToNextQuestion = useCallback(() => {
    ...
    setGameState((prev) => {
      ...
      let nextPhase: AgoraxGameState["phase"] = "turn-active";
      let roundTitle = prev.roundTitle;
      let roundType: AgoraxGameState["roundType"] = "turn-based";
  ...
  function handleStealAnswer(stealerPlayer: AgoraxPlayer, choiceIndex: number) {
  ...
  function handleProceedToLaLigne(finalist1: AgoraxPlayer, finalist2: AgoraxPlayer) {
    sound.playDouble();
    setGameState((prev) => {
      if (!prev) return null;
      return {
  ...
  if (loading) {
    return (
      <main className="mx-auto flex min-h-dvh max-w-xl flex-col items-center justify-center px-4 text-center">
        ...
        <h1 className="mt-4 text-[20px] font-semibold text-fp-text">Agorax</h1>
