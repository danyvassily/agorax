...
/**
 * Agorax — La Ligne (finale AGORAX)
 * 9 positions, curseur au centre, DOUBLE tous les 3 tours, chrono 90s.
 */
import { useEffect, useState, useRef } from "react";
import type { LaLigneState, AgoraxPlayer } from "@/lib/game/agorax-engine";
...
interface LaLigneProps {
  state: LaLigneState;
  finalist1: AgoraxPlayer;
  finalist2: AgoraxPlayer;
...
export function LaLigneGame({
  ...
  const expiryCallback = useRef(onTimeExpired);
  useEffect(() => { expiryCallback.current = onTimeExpired; }, [onTimeExpired]);

  // Chronomètre global de la finale (90s)
  useEffect(() => {
    if (state.winnerId) return;
    if (deadline.current === null) deadline.current = Date.now() + state.secondsRemaining * 1000;
    const timer = setInterval(() => {
      ...
      } else if (remaining <= 10 && remaining > 0) sound.playTick();
    }, 1000);
    return () => { clearInterval(timer); if (pending.current) clearTimeout(pending.current); };
  }, [state.winnerId, state.secondsRemaining]);

  function handleChoice(index: number) {
    if (answeredIndex !== null || pending.current || !currentQuestion || state.winnerId || expired.current) return;
  ...
  return (
    <div className="jx-finale mx-auto flex min-h-[78vh] w-full max-w-2xl flex-col items-center justify-between px-4 py-3">
      {/* Header */}
