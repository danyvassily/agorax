...
export function SettingsPage() {
  ...
  const { user, isLoggedIn, signOut, updateLanguage } = useAuth();
  const isFR = lang === "fr";
  ...
  return (
    <>
      ...
      {/* Langue & Quiz */}
      <p className="fp-group-title">{isFR ? "Langue & Quiz" : "Language & Quizzes"}</p>
      <div className="fp-list p-2.5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <button
            ...
            className={`flex items-center gap-3 rounded-xl border p-3 text-left transition ${
              lang === "fr"
                ? "border-fp-primary bg-fp-primary/10 shadow-xs"
                : "border-black/[0.06] bg-white hover:bg-black/[0.02]"
            }`}
          >
            <span className="text-2xl">🇫🇷</span>
            <div>
              <p className="text-sm font-bold text-fp-text">Français</p>
              <p className="text-[11px] text-fp-text-dim">Quiz et interface en français</p>
            </div>
          </button>
          <button
            type="button"
          ...
          >
            <span className="text-2xl">🇬🇧</span>
            <div>
              <p className="text-sm font-bold text-fp-text">English</p>
              <p className="text-[11px] text-fp-text-dim">Interface and auto-translated quizzes</p>
            </div>
          </button>
        </div>
      </div>
