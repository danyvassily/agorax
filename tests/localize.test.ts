...
describe("localizeQuestion", () => {
  ...
  it("traduit automatiquement en anglais si la traduction est absente", () => {
    const sansTrad = { ...Q, translations: undefined };
    const l = localizeQuestion(sansTrad, "en", { autoTranslate: true });
    expect(l.question).toBe("What is the capital of Australia?");
    expect(l.answers[1]).toBe("Canberra");
    expect(l.lang).toBe("en");
  });

  it("retombe sur le français si autoTranslate est désactivé", () => {
    const sansTrad = { ...Q, translations: undefined };
    const l = localizeQuestion(sansTrad, "en", { autoTranslate: false });
    expect(l.question).toBe(Q.question);
    expect(l.lang).toBe("fr");
  });
